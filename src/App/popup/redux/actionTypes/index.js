export const CHANGE_THEME = "CHANGE_THEME"; //  切换主题
export const SET_MARKET_STATE = "SET_MARKET_STATE"; //  市场状态 Boolean
export const SET_MARKET_STATE_TEXT = "SET_MARKET_STATE_TEXT"; //  市场状态文本 String

export const SET_ACTIVE_TR = "SET_ACTIVE_TR"; //  点击获取内容 & operation面板激活状态
export const SET_TOTAL_INCOME = "SET_TOTAL_INCOME"; // 计算总收益
export const SET_TOTAL_CREASE = "SET_TOTAL_CREASE"; //  涨跌幅

export const CHANGE_SEARCH_STATE = "CHANGE_SEARCH_STATE"; //  搜索状态a
export const SET_SEARCH_LOADING = "SET_SEARCH_LOADING"; //  搜索的loading
export const SET_SEARCH_RESULT = "SET_SEARCH_RESULT"; //  搜索结果
export const UPDATE_FORCE = "UPDATE_FORCE"; //  搜索完成后更新数据

export const SET_SORT_KEY = "SET_SORT_KEY"; //  需要排序的字段 key_0

export const SET_MENU_STATE = "SET_MENU_STATE"  //  菜单
export const TOGGLE_WIDESCREEN = "TOGGLE_WIDESCREEN" // 宽屏模式
export const SET_IMPORT_STATE = "SET_IMPORT_STATE"  //  导入数据面板状态
